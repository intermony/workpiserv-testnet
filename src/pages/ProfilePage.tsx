...
import {
  Wallet, Copy, Check, Pencil, X, LogOut, Package, AlertTriangle, ShieldCheck, Info,
      ...
      <div className="section-container py-6 space-y-4">
        ...
        <section className="card-surface p-5 space-y-4">
          ...
          <div className="rounded-2xl border border-red-200 bg-red-50 p-4 space-y-2">
            ...
            <ul className="space-y-1.5 text-xs text-red-700 list-none">
          ...
          {/* ── Explication format ── */}
          <div className="rounded-2xl border border-blue-100 bg-blue-50 p-3 flex gap-2">
            <Info size={14} className="text-blue-500 shrink-0 mt-0.5" />
            <p className="text-xs text-blue-700">{t('wallet.info.format')}</p>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                {draft.length > 0 && !draft.startsWith('G') && (
                  <span className="text-xs text-red-500">⚠️ Doit commencer par G</span>
                )}
              </div>
              {walletError && (
              ...
              {draft.length === 56 && draft.startsWith('G') && (
                <div className="rounded-xl bg-amber-50 border border-amber-200 p-3">
                  <p className="text-xs text-amber-800 font-medium mb-1">⚠️ {t('wallet.confirm.title')}</p>
                  <code className="text-[10px] text-amber-900 break-all block">{draft}</code>
                  <p className="text-xs text-amber-700 mt-1">{t('wallet.confirm.hint')}</p>
                </div>
              )}
              <div className="flex gap-2">
