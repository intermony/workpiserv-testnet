...
const Sidebar = ({ user, currentPath, navigate, lang, T }) => {
  ...
  const userTypeLabel = user
    ? (user.type==='freelancer' ? T.freelancerType : user.type==='admin' ? T.adminType : user.type==='both' ? (T.bothType||'Pionnier') : T.clientType)
    : T.notRegistered;
...
const LoginModal = ({ isOpen, onClose, onLogin, onPiLogin, T }) => {
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal login-modal" style={{textAlign:'center', background:'#1A1A2E', color:'#FFFFFF', maxWidth:'400px'}}>
        <div className="modal-header">
        ...
        {!showDemo ? (
          <>
            <p style={{color:'#8FA8B5', marginBottom:'2rem', fontSize:'0.9rem', lineHeight:1.6}}>
