...
const RoleSelectModal = ({ piUsername, onSelectRole, T }) => {
  ...
  const roles = [
    {
      value: 'client',
      icon: '🛍️',
      label: T.clientType || 'Client',
      desc: T.clientDesc || 'Je cherche des services et des freelancers',
      color: '#0077B6',
    ...
    },
    {
      value: 'freelancer',
      icon: '💼',
      label: T.freelancerType || 'Freelancer',
      desc: T.freelancerDesc || 'Je propose mes services et mes compétences',
      color: '#FF6B35',
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 9999 }}>
      <div className="modal login-modal" style={{
        textAlign: 'center', background: '#1A1A2E', color: '#FFFFFF', maxWidth: '420px'
      }}>
        <div className="modal-header">
          <h2 className="modal-title" style={{ color: '#FFD166', fontSize: '1.3rem' }}>
            🎉 {T.roleWelcome || 'Bienvenue'}, {piUsername} !
          </h2>
        </div>
        <p style={{ color: '#8FA8B5', marginBottom: '1.5rem', fontSize: '0.9rem' }}>
          {T.roleSubtitle || 'Vous souhaitez vous connecter en tant que :'}
        </p>
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
          {roles.map(r => (
            <div
              ...
              style={{
                flex: 1, border: `2px solid ${selected === r.value ? r.color : 'rgba(255,255,255,0.1)'}`,
                borderRadius: '16px', padding: '1.25rem', cursor: 'pointer',
                background: selected === r.value ? `${r.color}22` : 'rgba(255,255,255,0.03)',
                transition: 'all 0.25s',
                transform: selected === r.value ? 'translateY(-3px)' : 'none',
                boxShadow: selected === r.value ? `0 8px 24px ${r.color}40` : 'none',
              }}
            >
              <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{r.icon}</div>
              <div style={{
                fontWeight: 700, fontSize: '1rem', marginBottom: '0.4rem',
                color: selected === r.value ? r.color : '#f1f5f9'
              }}>{r.label}</div>
        ...
        >
          {loading ? '⏳' : selected
            ? `${selected === 'freelancer' ? '💼 ' + (T.freelancerType || 'Freelancer') : '🛍️ ' + (T.clientType || 'Client')}`
            : (T.roleSubtitle || 'Choisissez un rôle')}
        </button>
      </div>
    </div>
  );
...
function App() {
  ...
  const [pendingPiUser, setPendingPiUser] = useState(null); // nouvel utilisateur Pi en attente de rôle
  ...
  const handlePiLogin = async () => {
    try {
      ...
      // Nouvel utilisateur Pi — afficher le choix de rôle
      if (result.isNew) {
        setPendingPiUser({ piUserId: result.piUserId, username: result.username });
        setShowLogin(false);
        return;
      }
  ...
  // Enregistrement après choix de rôle (nouvel utilisateur Pi)
  const handlePiRegister = async (role) => {
    if (!pendingPiUser) return;
    try {
      const res = await api.post('/api/auth/pi/register', {
        piUserId: pendingPiUser.piUserId,
        username: pendingPiUser.username,
        role,
      });
      localStorage.setItem('token', res.data.token);
      localStorage.setItem('user', JSON.stringify(res.data.user));
      setUser(res.data.user);
      setPendingPiUser(null);
      addToast(`${T.welcomeMsg} ${res.data.user.username}!`, 'success');
    } catch { addToast(T.loginError || 'Erreur lors de l\'inscription', 'error'); }
  };
  ...
  return (
    ...
    <div className="app-layout" style={{ flex: 1 }}>
      ...
      {pendingPiUser && (
        <RoleSelectModal
          piUsername={pendingPiUser.username}
          onSelectRole={handlePiRegister}
          T={T}
        />
      )}
    </div>
