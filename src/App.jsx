...
const RoleSelectModal = ({ piUsername, onSelectRole }) => {
  ...
  const roles = [
    ...
    },
    {
      ...
      icon: '💼',
      label: 'Freelancer',
      desc: 'Je propose mes services et mes compétences',
      color: '#FF6B35',
      ...
      <div className="modal login-modal" style={{
        ...
        <div className="modal-header">
          <h2 className="modal-title" style={{ color: '#FFD166', fontSize: '1.4rem' }}>
            🎉 Bienvenue, {piUsername} !
          </h2>
        </div>
        <p style={{ color: '#8FA8B5', marginBottom: '1.5rem', fontSize: '0.9rem' }}>
          Première connexion détectée. Choisissez votre rôle sur WorkPiServ :
        </p>
        ...
        >
          {loading ? '⏳ Connexion...' : selected ? `Rejoindre en tant que ${selected === 'freelancer' ? 'Freelancer 💼' : 'Client 🛍️'}` : 'Choisissez un rôle'}
        </button>
        <p style={{ color: '#5A7A8A', fontSize: '0.75rem', marginTop: '1rem' }}>
          Vous pourrez changer de rôle depuis votre profil.
        </p>
      </div>
    </div>
  );
