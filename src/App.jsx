...
const LoginModal = ({ isOpen, onClose, onLogin, onPiLogin, T }) => {
  ...
  const [step, setStep] = useState(1); // step 1: role, step 2: name
      ...
      <div className="modal" style={{textAlign:'center', background:'#0f172a', color:'#f1f5f9'}}>
        <div className="modal-header">
          <h2 className="modal-title" style={{color:'#f1f5f9'}}>
            {step === 1 ? '👋 Qui êtes-vous ?' : `${type === 'freelancer' ? '💼' : '🛍️'} Connexion`}
          </h2>
          <button className="modal-close" onClick={onClose}>×</button>
        </div>
        ...
        {step === 2 && (
          <>
            <div style={{
              ...
              {type==='freelancer' ? '💼 Freelancer' : '🛍️ Client'}
            </div>
