...
const HomePage = ({ navigate, T, user }) => {
  ...
  return (
    <div className="animate-fade">
      ...
      {/* ── Cartes d'action pionnier connecté ── */}
      {user && (
        <div style={{display:'flex', gap:'1rem', marginBottom:'2rem', flexWrap:'wrap'}}>
          <div onClick={() => navigate('/marketplace')} style={{
            flex:1, minWidth:'140px', background:'linear-gradient(135deg,#004E64,#0077B6)',
            borderRadius:'20px', padding:'1.5rem', cursor:'pointer', textAlign:'center',
            boxShadow:'0 8px 24px rgba(0,78,100,0.3)', transition:'transform 0.2s',
          }}>
            <div style={{fontSize:'2.5rem', marginBottom:'0.5rem'}}>🛍️</div>
            <div style={{fontWeight:700, fontSize:'1rem', color:'#fff', marginBottom:'0.25rem'}}>
              {T.findService || 'Chercher un service'}
            </div>
            <div style={{fontSize:'0.75rem', color:'rgba(255,255,255,0.7)'}}>
              {T.findServiceDesc || "Trouvez le talent qu'il vous faut"}
            </div>
          </div>
          <div onClick={() => navigate('/new-service')} style={{
            flex:1, minWidth:'140px', background:'linear-gradient(135deg,#FF6B35,#E55A28)',
            borderRadius:'20px', padding:'1.5rem', cursor:'pointer', textAlign:'center',
            boxShadow:'0 8px 24px rgba(255,107,53,0.3)', transition:'transform 0.2s',
          }}>
            <div style={{fontSize:'2.5rem', marginBottom:'0.5rem'}}>💼</div>
            <div style={{fontWeight:700, fontSize:'1rem', color:'#fff', marginBottom:'0.25rem'}}>
              {T.publishService || 'Publier un service'}
            </div>
            <div style={{fontSize:'0.75rem', color:'rgba(255,255,255,0.7)'}}>
              {T.publishServiceDesc || 'Proposez vos compétences'}
            </div>
          </div>
        </div>
      )}
