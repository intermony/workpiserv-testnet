...
export function usePiAuth(): UsePiAuthReturn {
  ...
  const login = useCallback(async (): Promise<boolean> => {
    ...
    // Termine la connexion à partir d'un résultat d'authentification
    const finalize = async (result: Awaited<ReturnType<typeof piSDK.authenticate>> | null): Promise<boolean> => {
    ...
    try {
      const authPromise = piSDK.authenticate();
      ...
      // Délai dépassé : dans le Pi Browser, l'écran de consentement est
      // peut-être encore ouvert. Si la réponse arrive plus tard, on termine
      // la connexion quand même (la fenêtre QR se fermera automatiquement).
      authPromise
        .then(late => { finalize(late).catch(() => {}); })
